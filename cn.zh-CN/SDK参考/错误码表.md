...
|0x0103030d|16974605|failed to join channel due to invalid channel profile|channel profile不匹配，如第一个人以通讯模式入会，之后如果有人已互动模式入会就会报此错。|重新检查用户入会模式和其他用户的入会模式（频道模式）是否一致。|
...
|0x0501001|83951642|join channel due to invaild channel id|AppId配置了大方会模式，需要使用1.16.5以上的包入会，否则会入会失败。|检查SDK版本。|
