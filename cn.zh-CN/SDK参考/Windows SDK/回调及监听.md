...
|[onFirstLocalVideoFrameDrawn](#li_a3y_r4w_dpq)|预览开始显示第一帧视频帧时触发此消息|1.17.19|
|[onRTCLocalAudioStats](#li_a02_ayr_hal)|本地音频统计信息|1.17.19|
|[onRTCRemoteAudioStats](#li_yhj_byk_r0n)|远端音频统计信息|1.17.19|
|[onActiveSpeaker](#li_hdh_nw6_p12)|订阅当前正在说话的人|1.17.19|
|[onGetVideoFormatPreference](#li_1qr_orp_8b9)|视频数据输出格式|1.17.19|
|[onGetVideoObservedFramePosition](#li_ppq_gzh_5p8)|视频数据输出位置|1.17.19|
|[onPreEncodeVideoSample](#li_8gl_km2_e4v)|本地编码前视频数据回调|1.17.19|
|[onConnectionStatusChange](#li_dus_vt9_ptx)|网络连接状态改变的回调|1.17.19|
|[onAudioDeviceRecordLevel](#li_6xk_81y_7h9)|麦克风设备测试音量回调|1.17.19|
|[onAudioDevicePlayoutLevel](#li_fba_bk4_8s8)|扬声器设备测试音量回调|1.17.19|
|[onAudioDevicePlayoutEnd](#li_dvr_qfn_m5d)|扬声器设备测试播放结束回调|1.17.19|
...
-   onFirstLocalVideoFrameDrawn：预览开始显示第一帧视频帧时触发此消息。

    ```
    virtual void onFirstLocalVideoFrameDrawn() {}
    ```

-   onRTCLocalAudioStats：本地音频统计信息，SDK每两秒触发一次此统计信息回调。

    ```
    virtual void onRTCLocalAudioStats(const AliRtcLocalAudioStats& localAudioStats) {};
    ```

    |参数|类型|描述|
    |--|--|--|
    |localAudioStats|const AliRtcLocalAudioStats&|本地音频统计信息。|

-   onRTCRemoteAudioStats：远端音频统计信息，SDK每两秒触发一次此统计信息回调。

    ```
    virtual void onRTCRemoteAudioStats(const AliRtcRemoteAudioStats& remoteAudioStats) {};
    ```

    |参数|类型|描述|
    |--|--|--|
    |remoteAudioStats|const AliRtcRemoteAudioStats&|远端音频统计信息。|
...
-   onActiveSpeaker：订阅当前正在说话的人。
    ...
    |参数|类型|描述|
    |--|--|--|
    |uid|const char \*|说话人。当uid为0表示本地说话人。其返回的是当前时间段内声音最大的用户ID，而不是瞬时声音最大的用户ID。|

-   onGetVideoFormatPreference：视频数据输出格式，在注册registerVideoSampleObserver后触发回调，应用可返回期望输出的视频数据格式。

    ```
    virtual AliRtcVideoFrameType onGetVideoFormatPreference() { return AliRtcYUV420; }
    ```

-   onGetVideoObservedFramePosition：视频数据输出位置，在注册registerVideoSampleObserver后触发回调，应用可返回期望输出的视频内容，对应数据将分别从onCaptureVideoSample/onPreEncodeVideoSample/onRemoteVideoSample。

    ```
    virtual uint32_t onGetVideoObservedFramePosition() { return static_cast<unsigned int>(AliRtcPositionPostCapture | AliRtcPositionPreRender | AliRtcPositionPreEncoder); };
    ```

-   onPreEncodeVideoSample：本地编码前视频数据回调。

    ```
    virtual void onPreEncodeVideoSample(AliRtcVideoSource videoSource, AliRtcVideoDataSample *videoSample) {};
    ```
...
-   onConnectionStatusChange：网络连接状态改变的回调。
    ...
    ```
    virtual void onConnectionStatusChange(AliRtcConnectionStatus status, AliRtcConnectionStatusChangeReason reason) {};
    ```

    |参数|类型|描述|
    |--|--|--|
    |status|[AliRtcConnectionStatus](/cn.zh-CN/SDK参考/Windows SDK/数据类型.md)|当前状态值，对应枚举值请参见：[AliRtcConnectionStatus](/cn.zh-CN/SDK参考/Windows SDK/数据类型.md)。|
    |reason|[AliRtcConnectionStatusChangeReason](/cn.zh-CN/SDK参考/Windows SDK/数据类型.md)|引起状态变化的具体原因，对应枚举值请参见：[AliRtcConnectionStatusChangeReason](/cn.zh-CN/SDK参考/Windows SDK/数据类型.md)。|
...
-   OnAudioDeviceRecordLevel：麦克风设备测试音量回调。
    ...
    |参数|类型|描述|
    |--|--|--|
    |level|int|音量值，范围为0~100。|

-   OnAudioDevicePlayoutLevel：扬声器设备测试音量回调。
...
-   OnAudioDevicePlayoutEnd：扬声器设备测试播放结束回调。
