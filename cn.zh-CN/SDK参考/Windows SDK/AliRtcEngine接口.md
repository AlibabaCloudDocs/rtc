...
-   joinChannel：加入频道。
    ...
    |参数|类型|描述|
    |--|--|--|
    |authInfo|const [AliRtcAuthInfo](cn.zh-CN/SDK参考/Windows SDK/数据类型.md#)&|鉴权信息，从App Server下发，APP Server可通过API获取。|
...
-   configLocalSimulcast：设置是否允许发布次要视频流。默认为允许发布次要视频流，手动发布时，需要调用publish才能生效。
    ...
    |track|[AliRtcVideoTrack](cn.zh-CN/SDK参考/Windows SDK/数据类型.md#)|流类型，当前只支持相机流：AliVideoTrackCamera。|
...
-   subscribe：手动订阅视频和音频流。返回为0时说明接口执行正常，但是否订阅成功还得看回调结果；返回为非0时，说明接口执行异常中断，订阅失败。
    ...
    |--|--|--|
    |uid|const AliRtc::String&|用户ID，从App server分配的唯一标示符。|
    |onResult|void\(\)\(void opaquePtr, const AliRtc::String& uid, [AliRtcVideoTrack](cn.zh-CN/SDK参考/Windows SDK/数据类型.md#) vt, [AliRtcAudioTrack](cn.zh-CN/SDK参考/Windows SDK/数据类型.md#) at\)|当subscribe执行结束后回调。|
    |opaquePtr|void \*|app提供的UserData，在调用onResult时传回app。|
...
-   setLocalViewConfig：为本地预览设置渲染窗口以及绘制参数。
    ...
    |track|[AliRtcVideoTrack](cn.zh-CN/SDK参考/Windows SDK/数据类型.md#)|预览只允许AliVideoTrackCamera。|
...
-   muteLocalCamera：设置是否停止发布本地视频流。不改变当前视频流的采集状态。
    ...
    |track|[AliRtcVideoTrack](cn.zh-CN/SDK参考/Windows SDK/数据类型.md#)|需要改变发布状态的videoTrack类型。|
...
-   setRemoteViewConfig：为远端的视频设置渲染窗口以及绘制参数。
    ...
    |uid|const AliRtc::String&|用户ID，从App server分配的唯一标示符。|
    |track|[AliRtcVideoTrack](cn.zh-CN/SDK参考/Windows SDK/数据类型.md#)|需要设置的videoTrack类型。|
...
-   setLogLevel：设置log级别。
    ...
    |参数|类型|说明|
    |--|--|--|
    |logLevel|[AliRtcLogLevel](cn.zh-CN/SDK参考/Windows SDK/数据类型.md#)|log级别。|
