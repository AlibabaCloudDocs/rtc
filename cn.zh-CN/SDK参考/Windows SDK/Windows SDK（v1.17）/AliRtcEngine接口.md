...
|[configLocalScreenPublish](#li_6i3_uhw_k3q)|设置是否允许发布屏幕流。|1.1|
...
-   sharedInstance：创建AliRTCEngine实例。
    ...
    |名称|类型|描述|
    |--|--|--|
    |listener|[AliRtcEventListener\*](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/回调及监听.md)|AliRtcEngine回调的监听器。|
...
-   joinChannel：加入频道。
    ...
    |名称|类型|描述|
    |--|--|--|
    |authInfo|const [AliRtcAuthInfo](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)&|鉴权信息。|
...
-   setChannelProfile：设置频道模式。
    ...
    |名称|类型|描述|
    |--|--|--|
    |channelProfile|[AliRtcChannelProfile](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|频道类型。默认为AliRtcCommunication。|
...
-   configLocalSimulcast：设置是否允许发布次要视频流。
    ...
    |track|[AliRtcVideoTrack](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|流类型，当前只支持AliVideoTrackCamera（相机流）。|
...
-   subscribe：手动订阅视频和音频流。
    ...
    |--|--|--|
    |uid|const AliRtc::String&|用户ID。|
    |onResult|void\(\)\(void opaquePtr, const AliRtc::String& uid, [AliRtcVideoTrack](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md) vt, [AliRtcAudioTrack](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md) at\)|调用subscribe执行结束后回调。|
    |opaquePtr|void \*|App提供的UserData，在调用onResult时传回App。|
...
-   subscribeAudioData：订阅音频数据。默认不订阅音频数据。
    ...
    |名称|类型|描述|
    |--|--|--|
    |audioSource|[AliRtcAudioSource](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|音频数据源。|
...
-   setVideoProfile：设置视频流的参数。
    ...
    |名称|类型|描述|
    |--|--|--|
    |profile|[AliRtcVideoProfile](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|视频流参数。默认为AliRtcVideoProfile\_Default。|
    |track|[AliRtcVideoTrack](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|需要设置的视频流类型。|
...
-   setLocalViewConfig：为本地预览设置渲染窗口以及绘制参数。
    ...
    |名称|类型|描述|
    |--|--|--|
    |canvas|const [AliVideoCanvas](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)&|渲染参数，包含渲染窗口以及渲染方式。|
    |track|[AliRtcVideoTrack](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|视频Track的类型，预览只允许AliVideoTrackCamera（摄像头流）。|
...
-   muteLocalCamera：设置是否停止发布本地视频流。
    ...
    |track|[AliRtcVideoTrack](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|需要改变发布状态的视频Track类型。|
...
-   setRemoteViewConfig：为远端的视频设置渲染窗口以及绘制参数。
    ...
    |名称|类型|描述|
    |--|--|--|
    |canvas|[AliVideoCanvas](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|渲染参数，包含渲染窗口以及渲染方式。|
    |uid|const AliRtc::String&|用户ID。|
    |track|[AliRtcVideoTrack](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|需要设置的视频Track类型。|
...
-   getScreenShareSourceInfo：获取屏幕分享源信息。
    ...
    |名称|类型|描述|
    |--|--|--|
    |source\_type|[AliRtcScreenShareType](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|桌面分享类型。|
    |source\_list|[AliRtcScreenSourceList](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|屏幕分享数据源信息。|
...
-   setExternalVideoSource：启用外部视频输入源。
    ...
    |sourceType|[AliRtcVideoSource](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|流类型。|
    |renderMode|[AliRtcRenderMode](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|渲染模式。|
...
-   pushExternalVideoFrame：输入外部视频。
    ...
    |名称|类型|描述|
    |--|--|--|
    |frame|[AliRtcVideoDataSample \*](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|帧数据。|
    |type|[AliRtcVideoSource](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|流类型。|
...
-   startRecord：开始录制。
    ...
    |名称|类型|说明|
    |--|--|--|
    |recordType|[AliRtcRecordType](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|录制类型。|
    |recordFormat|[AliRtcRecordFormat](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|录制格式。|
    |filePath|const AliRtc::String&|文件路径。|
    |audioConfig|[AliRtcRecordAudioConfig&](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|录制音频设置。|
    |videoConfig|[AliRtcRecordVideoConfig&](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|录制视频设置。|
...
-   setBeatutyEffect：设置是否启用基础美颜。
    ...
    |config|[AliRtcBeautyConfig](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|基础美颜参数。|
...
-   setVideoEncoderConfiguration： 设置视频编码属性。
    ...
    |名称|类型|描述|
    |--|--|--|
    |config|[AliRtcVideoEncoderConfiguration](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|预定义的视频编码属性。|
...
-   setScreenShareSourceByRegion：通过指定区域设置屏幕分享源。
    ...
    **说明：** 此方法仅适用于桌面共享，设置窗口共享请使用[setScreenShareSource](#li_f8l_0uk_xza)接口。
    ...
    |名称|类型|描述|
    |--|--|--|
    |screenRect|[AliRtcScreenShareRegion](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|指定要共享的屏幕相对于虚拟屏幕的位置。虚拟屏幕位置请参见[The Virtual Screen](https://docs.microsoft.com/en-us/windows/win32/gdi/the-virtual-screen)。|
    |isShareByRegion|bool|是否只分享指定屏幕内特定区域。|
    |regionRect|[AliRtcScreenShareRegion](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|指定共享屏幕内要分享的区域（isShareByRegion为true时才需赋值）。**说明：**
...
-   setScreenShareSource：设置屏幕分享源。
    ...
    |名称|类型|描述|
    |--|--|--|
    |source|[AliRtcScreenSource](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|屏幕分享源信息。|
...
-   setSubscribeAudioNumChannel：设置输出音频声道数。
    ...
    |名称|类型|描述|
    |--|--|--|
    |audioNumChannel|[AliRtcAudioNumChannelType](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|音频声道类型。默认为单声道。|
...
-   setSubscribeAudioSampleRate：设置输出音频采样率。
    ...
    |名称|类型|描述|
    |--|--|--|
    |audioSampleRate|[AliRtcAudioSampleRate](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|音频采样率。默认为44.1k。|
...
-   setAudioEffectReverbMode：设置混响音效模式。
    ...
    |名称|类型|说明|
    |--|--|--|
    |mode|const [AliRtcAudioEffectReverbMode](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|对应混响音效类型。默认无混响模式。|
...
-   setAudioEffectReverbParamType：设置混响音效类型。
    ...
    |名称|类型|说明|
    |--|--|--|
    |type|const [AliRtcAudioEffectReverbParamType](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|对应混响音效类型。|
...
-   createMediaDeviceTestInterface：创建音视频设备测试实例。
    ...
    |名称|类型|描述|
    |--|--|--|
    |pMediaDeviceEventListener|[AliMediaDeviceTestEventListener \*](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/回调及监听.md)|音频设备测试事件监听器。|
...
-   setLogLevel：设置日志级别。
    ...
    |名称|类型|描述|
    |--|--|--|
    |logLevel|[AliRtcLogLevel](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|日志级别。默认取值AliRtcLogLevelInfo。|
...
-   setClientRole：设置用户角色。返回0为成功，否则返回错误码。
    ...
    |名称|类型|描述|
    |--|--|--|
    |clientRole|[AliRtcClientRole](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|用户角色。默认为AliRtcClientRoleLive。|
...
-   startLastmileDetect：开始网络质量探测。
    ...
    **说明：** 您需要在加入频道之前调用，并且入会后会自动停止，探测结果在[onLastmileDetectResultWithQuality](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/回调及监听.md)回调。
...
-   postFeedback：SDK问题反馈。
    ...
    |type|[AliRtcFeedbackType](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|问题类型。|
...
-   sendMediaExtensionMsg：发送媒体扩展信息。
    ...
    **说明：** 使用音视频数据通道，将自定义消息发送给房间内其他用户，需要满足两个前提：
    ...
    -   己方正常入会，并且在推流中。
    -   房间内的其他用户需要订阅己方音视频流，发送成功之后可在[onMediaExtensionMsgReceived](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/回调及监听.md)回调中接收结果。
    ```
...
-   RefreshAuthInfo：刷新令牌。
    ...
    |名称|类型|描述|
    |--|--|--|
    |authInfo|[AliRtcAuthInfo](/cn.zh-CN/SDK参考/Windows SDK/Windows SDK（v1.17）/数据类型.md)|新的鉴权信息。|
