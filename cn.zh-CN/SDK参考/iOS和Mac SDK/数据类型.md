...
-   AliRtcVideoObserPosition：视频数据输出位置。
    ...
    |枚举名|描述|
    |---|--|
    |AliRtcPositionPostCapture|采集视频数据，对应输出回调[onCaptureVideoSample]()。|
    |AliRtcPositionPreRender|渲染视频数据，对应输出回调[onRemoteVideoSample]()。|
    |AliRtcPositionPreEncoder|编码前视频数据，对应输出回调[onPreEncodeVideoSample]()。|
