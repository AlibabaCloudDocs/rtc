...
|[onUserVideoEnabled](#li_035)|禁用或重新启用本地视频采集的通知通知。|2.1|
...
|[onTextureCreate](#li_040)|订阅本地视频Texture创建回调。|2.1|
|[onTextureUpdate](#li_041)|订阅本地视频Texture绘制回调。|2.1|
|[onTextureDestory](#li_042)|订阅本地视频Texture销毁回调。|2.1|
...
|[onDownlinkMessageNotify](#li_053)|下行消息通道（接收消息）。|2.1|
|[onUplinkMessageResponse](#li_054)|发送上行消息后返回结果。|2.1|
|[onAudioRouteChanged](#li_055)|语音路由发生变化回调。|2.1|
|[onSnapshotComplete](#li_056)|截图回调。|2.1|
|[onPublishLiveStreamStateChanged](#li_057)|旁路推流状态改变回调。|2.1|
|[onPublishTaskStateChanged](#li_058)|旁路任务状态改变回调。|2.1|
...
-   onJoinChannelResult：加入频道结果回调（该回调等同于调用[joinChannel]()接口的block操作，即处理加入频道之后的事件，选择其一即可）。
...
-   onLeaveChannelResult：离开频道结果回调。调用[leaveChannel]()接口后返回，如果调用[leaveChannel]()后直接调用[destroy]()，将不会收到此回调。
...
-   onGetVideoFormatPreference：视频数据输出格式的回调。
    ...
    返回期望输出的视频数据格式，默认返回AliRtcYUV420。需要在注册[registerVideoSampleObserver]()后触发回调。
...
-   onUserVideoEnabled：禁用或重新启用本地视频采集的通知。
    ...
    |参数|类型|描述|
    |--|--|--|
    |uid|NSString \*\_Nonnull|执行执行EnableLocalVideo的用户的用户ID。|
...
-   onTextureCreate：订阅本地视频Texture创建回调。
...
-   onTextureUpdate：订阅本地视频Texture绘制回调。
...
-   onTextureDestory：订阅本地视频Texture销毁回调。
...
-   onDownlinkMessageNotify：下行消息通道（接收消息）。
...
-   onPublishLiveStreamStateChanged：旁路推流状态改变回调。
    ...
    |参数|类型|描述|
    |--|--|--|
    |streamURL|NSString \*\_Nonnull|流地址。|
    |state|[AliRtcTrascodingLiveStreamStatus](/cn.zh-CN/SDK参考/iOS和Mac SDK/数据类型.md)|状态。|
...
-   onUpdateRoleNotifyWithOldRole：用户角色发生改变时回调（调用[setClientRole]()方法切换角色成功时触发此回调）。
